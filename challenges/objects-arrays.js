...
zooAnimals.forEach(function(el){
  return displayNames.push(`Name: ${el.animal_name}, Scientific Name:  ${el.scientific_name}`);
})
...
zooAnimals.map(function(el){
  return lowCaseAnimalNames.push(`${el.animal_name.toLowerCase()}`);
})
...
zooAnimals.filter(function(el){
  if (el.population < 5){
    return lowPopulationAnimals.push(el);
  }
})
...
const populationTotal = zooAnimals.reduce(function(accumulator, current){
  return current.population + accumulator;
}, 0);
